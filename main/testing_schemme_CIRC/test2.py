...
def generate_circ(logic_function: str, output_file: str):
    ...
    for op, idx in operations:
        ...
        input1 = input_pins.get(tokens[idx - 2])
        input2 = input_pins.get(tokens[idx - 1])

        if input1 and input2:
            ET.SubElement(main_circuit, 'wire', from_=f"({input1[0] + 20},{input1[1]})", to=f"({x},{y})")
            ET.SubElement(main_circuit, 'wire', from_=f"({input2[0] + 20},{input2[1]})", to=f"({x},{y})")
        
        operation_comps.append((op, (x, y)))
        output_pins.append((x + 20, y))
        x += 100
...
generate_circ("a AND b OR c", "output2.circ")
