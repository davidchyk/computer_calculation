...
from normalmini import minimized_implicants
...
def test_compare(num_vars: int, values: list[int], is_dnf: bool):
    ...
    if is_dnf:
        ...
        result_qmc = minimized_implicants(num_vars, values, is_dnf=True)
        form = "МДНФ"
    else:
        implicants_sympy = minimize_cnf_as_implicants(num_vars, [x for x in range(number_of_sets) if x not in values])[1]
        result_qmc = minimized_implicants(num_vars, [x for x in range(number_of_sets) if x not in values], is_dnf=False)
        form = "МКНФ"
...
if __name__ == '__main__':
    ...
    test_compare(num_vars=9, values=[1, 2, 3], is_dnf=True)
    ...
    test_compare(num_vars=9, values=[1, 2, 3], is_dnf=False)